    ...
    static public class Music extends OpenALMusic {
        ...
        public int read(byte[] buffer) {
            ...
            try {
                ...
                throw new KermanRuntimeException("Error reading WAV file: " + file, ex);
            }
        }
        ...
        public WavInputStream(FileHandle file) {
            ...
            try {
                if (read() != 'R' || read() != 'I' || read() != 'F' || read() != 'F')
                    throw new KermanRuntimeException("RIFF header not found: " + file);

                skipFully(4);

                if (read() != 'W' || read() != 'A' || read() != 'V' || read() != 'E')
                    throw new KermanRuntimeException("Invalid wave file header: " + file);
                ...
                if (type != 0x0001 && type != 0x0003) throw new KermanRuntimeException(
                ...
                if (type == 0x0001) { // PCM
                    if (bitDepth != 8 && bitDepth != 16)
                        throw new KermanRuntimeException("PCM WAV files must be 8 or 16-bit: " + bitDepth);
                } else if (type == 0x0003) { // Float
                    if (bitDepth != 32 && bitDepth != 64)
                        throw new KermanRuntimeException("Floating-point WAV files must be 32 or 64-bit: " + bitDepth);
                }
