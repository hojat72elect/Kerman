...
import com.kerman.core.scenes.scene2d.utils.ChangeListener.ChangeEvent;
...
import com.kerman.core.scenes.scene2d.utils.Selection;
import com.kerman.core.scenes.scene2d.utils.UIUtils;
import com.kerman.core.utils.KermanArray;
import org.jetbrains.annotations.Nullable;

/**
 * Info : This class was inspired by "com.badlogic.gdx.scenes.scene2d.ui.Tree".
 * <p>
...
public class Tree<N extends Tree.Node, V> extends WidgetGroup implements Styleable<Tree.TreeStyle> {
    ...
    final KermanArray<N> rootNodes = new KermanArray<>();
    ...
    static boolean findExpandedValues(KermanArray<? extends Node> nodes, KermanArray values) {
    ...
    static @Nullable Node findNode(KermanArray<? extends Node> nodes, Object value) {
        for (int i = 0, n = nodes.size; i < n; i++) {
            Node node = nodes.get(i);
    ...
    static void collapseAll(KermanArray<? extends Node> nodes) {
        for (int i = 0, n = nodes.size; i < n; i++) {
            Node node = nodes.get(i);
    ...
    static void expandAll(KermanArray<? extends Node> nodes) {
        for (int i = 0, n = nodes.size; i < n; i++)
    ...
    private void computeSize(KermanArray<N> nodes, float indent, float plusMinusWidth) {
    ...
    private float layout(KermanArray<N> nodes, float indent, float y, float plusMinusWidth) {
        float ySpacing = this.ySpacing;
     ...
     */
    protected float drawIcons(Batch batch, float r, float g, float b, float a, @Nullable N parent, KermanArray<N> nodes, float indent,
    ...
    protected Drawable getExpandIcon(N node, float iconX) {
        if (node == overNode
                && Kerman.app.getType() == ApplicationType.Desktop
                && (!selection.getMultiple() || (!UIUtils.ctrl() && !UIUtils.shift()))
        ) {
            float mouseX = screenToLocalCoordinates(tmp.set(Kerman.input.getX(), 0)).x + getX();
    ...
    private float getNodeAt(KermanArray<N> nodes, float y, float rowY) {
        for (int i = 0, n = nodes.size; i < n; i++) {
            N node = nodes.get(i);
            float height = node.height;
    ...
    void selectNodes(KermanArray<N> nodes, float low, float high) {
        for (int i = 0, n = nodes.size; i < n; i++) {
            N node = nodes.get(i);
     ...
     * correct order.
     */
    public KermanArray<N> getRootNodes() {
        return rootNodes;
    }
    ...
    public void findExpandedValues(KermanArray<V> values) {
        findExpandedValues(rootNodes, values);
    }

    public void restoreExpandedValues(KermanArray<V> values) {
    ...
    static abstract public class Node<N extends Node, V, A extends Actor> {
        final KermanArray<N> children = new KermanArray<>(0);
        A actor;
        N parent;
        ...
        public void addAll(KermanArray<N> nodes) {
            for (int i = 0, n = nodes.size; i < n; i++)
         ...
         */
        public KermanArray<N> getChildren() {
            return children;
        }
