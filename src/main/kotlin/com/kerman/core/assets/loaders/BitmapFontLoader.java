...
/**
 * Info : This class was inspired by "com.badlogic.gdx.assets.loaders.BitmapFontLoader".
 * <p>
...
public class BitmapFontLoader extends AsynchronousAssetLoader<BitmapFont, BitmapFontLoader.BitmapFontParameter> {
    ...
    @Override
    public KermanArray<AssetDescriptor> getDependencies(String fileName, FileHandle file, BitmapFontParameter parameter) {
    ...
    public BitmapFont loadSync(AssetManager manager, String fileName, FileHandle file, BitmapFontParameter parameter) {
        if (parameter != null && parameter.atlasName != null) {
            ...
            if (region == null)
                throw new KermanRuntimeException("Could not find font region " + name + " in atlas " + parameter.atlasName);
            return new BitmapFont(file, region);
        } else {
            int n = data.getImagePaths().length;
            KermanArray<TextureRegion> regs = new KermanArray<>(n);
            for (int i = 0; i < n; i++) {
