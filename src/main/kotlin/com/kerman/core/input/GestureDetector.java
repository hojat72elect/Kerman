...
import com.kerman.core.InputProcessor;
import com.kerman.core.math.Vector2;
import com.kerman.core.utils.TimeUtils;
import com.kerman.core.utils.Timer;
import com.kerman.core.utils.Timer.Task;

/**
 * Info : This class was inspired by "com.badlogic.gdx.input.GestureDetector".
 * <p>
...
public class GestureDetector extends InputAdapter {
    ...
    public boolean touchDown(float x, float y, int pointer, int button) {
        ...
        if (pointer == 0) {
            pointer1.set(x, y);
            touchDownTime = Kerman.input.getCurrentEventTime();
            tracker.start(x, y, touchDownTime);
            if (Kerman.input.isTouched(1)) {
                // Start pinch.
    ...
    public boolean touchDragged(float x, float y, int pointer) {
        ...
        // update tracker
        tracker.update(x, y, Kerman.input.getCurrentEventTime());
    ...
    public boolean touchUp(float x, float y, int pointer, int button) {
        ...
        if (pinching) {
            ...
            if (pointer == 0) {
                ...
                tracker.start(pointer2.x, pointer2.y, Kerman.input.getCurrentEventTime());
            } else {
                // second pointer has lifted off, set up panning to use the first pointer...
                tracker.start(pointer1.x, pointer1.y, Kerman.input.getCurrentEventTime());
            }
            return false;
        }
        ...
        // handle fling
        long time = Kerman.input.getCurrentEventTime();
