...
/**
 * Info : This class was inspired by "com.badlogic.gdx.utils.Timer".
 * <p>
...
public class Timer {
    ...
    final KermanArray<Task> tasks = new KermanArray<>(false, 8);
    long stopTimeMillis;
    ...
    static TimerThread thread() {
        synchronized (threadLock) {
            if (thread == null || thread.files != Kerman.files) {
    ...
    public void start() {
        synchronized (threadLock) {
            TimerThread thread = thread();
            KermanArray<Timer> instances = thread.instances;
    ...
    static abstract public class Task implements Runnable {
        ...
        public Task() {
            app = Kerman.app; // Store which app to postRunnable (eg for multiple LwjglAWTCanvas).
    ...
    static class TimerThread implements Runnable, LifecycleListener {
        final Files files;
        final Application app;
        final KermanArray<Timer> instances = new KermanArray<>(1);
        final KermanArray<Task> postedTasks = new KermanArray<>(2);
        final KermanArray<Task> runTasks = new KermanArray<>(2);
        ...
        public void run() {
            while (true) {
                synchronized (threadLock) {
                    if (thread != this || files != Kerman.files) break;
                    ...
                    if (pauseTimeMillis == 0) {
                        ...
                        for (int i = 0, n = instances.size; i < n; i++) {
                            try {
                                ...
                                throw new KermanRuntimeException("Task failed: " + instances.get(i).getClass().getName(), ex);
                            }
                        }
                    }
