...
/**
 * Info : This class was inspired by "com.badlogic.gdx.utils.Select".
 * <p>
 ...
 * in a {@link KermanRuntimeException}
 * </p>
 * <p>
...
public class Select {
    ...
    /**
     * Provided for convenience. The instance of the "Select" class will be a singleton.
     */
    ...
    public <T> int selectIndex(T[] items, Comparator<T> comp, int kthLowest, int size) {
        if (size < 1) {
            throw new KermanRuntimeException("cannot select from empty array (size < 1)");
        } else if (kthLowest > size) {
            throw new KermanRuntimeException("Kth rank is larger than size. k: " + kthLowest + ", size: " + size);
        }
        int idx;
