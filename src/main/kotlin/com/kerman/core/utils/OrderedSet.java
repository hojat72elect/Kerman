...
/**
 * Info : This class was inspired by "com.badlogic.gdx.utils.OrderedSet".
 * <p>
 * An {@link ObjectSet} that also stores keys in a {@link KermanArray} using the insertion order. Null keys are not allowed. No
 ...
 * ObjectSet and the order does not actually matter, copying during remove can be greatly reduced by setting {@link KermanArray#ordered}
...
public class OrderedSet<T> extends ObjectSet<T> {
    final KermanArray<T> items;
    transient OrderedSetIterator iterator1, iterator2;

    public OrderedSet() {
        items = new KermanArray<>();
    }

    public OrderedSet(int initialCapacity, float loadFactor) {
        super(initialCapacity, loadFactor);
        items = new KermanArray<>(initialCapacity);
    }
    ...
    public OrderedSet(OrderedSet<? extends T> set) {
        super(set);
        items = new KermanArray<>(set.items);
    }
    ...
    public KermanArray<T> orderedItems() {
        return items;
    }
    ...
    static public class OrderedSetIterator<K> extends ObjectSetIterator<K> {
        private final KermanArray<K> items;
        ...
        public KermanArray<K> toArray() {
            return toArray(new KermanArray<>(true, set.size - nextIndex));
        }
    }
}
