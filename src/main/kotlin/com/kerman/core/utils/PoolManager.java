...
import com.kerman.core.utils.DefaultPool.PoolSupplier;

/**
 * Info : This class was inspired by "com.badlogic.gdx.utils.PoolManager".
 * <p>
...
public class PoolManager {
    ...
    public <T> void addPool(Pool<T> pool) {
        ...
        if (oldPool != null) {
            throw new KermanRuntimeException("Attempt to add pool with already existing class: " + clazz
    ...
    public <T> Pool<T> getPool(Class<T> clazz) {
        ...
        if (pool == null) {
            throw new KermanRuntimeException("Attempt to get pool with unknown class: " + clazz
                    + ", register using PoolManager#addPool(" + clazz.getSimpleName() + "::new)");
        }
        return pool;
    }
    ...
    public <T> T obtain(Class<T> clazz) {
        Pool<T> pool = (Pool<T>) typePools.get(clazz);
        if (pool == null) {
            throw new KermanRuntimeException("Attempt to get pooled object with unknown class: " + clazz
                    + ", register using PoolManager#addPool(" + clazz.getSimpleName() + "::new)");
        }
        return pool.obtain();
    }
    ...
    public <T> void free(T object) {
        ...
        if (pool == null) {
            throw new KermanRuntimeException("Attempt to free pooled object with unknown class: " + object.getClass()
