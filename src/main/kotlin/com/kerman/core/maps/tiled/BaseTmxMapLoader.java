...
import com.kerman.core.maps.MapGroupLayer;
import com.kerman.core.maps.MapLayer;
import com.kerman.core.maps.MapLayers;
import com.kerman.core.maps.MapObject;
import com.kerman.core.maps.MapObjects;
import com.kerman.core.maps.MapProperties;
import com.kerman.core.maps.objects.EllipseMapObject;
import com.kerman.core.maps.objects.PointMapObject;
import com.kerman.core.maps.objects.PolygonMapObject;
import com.kerman.core.maps.objects.PolylineMapObject;
import com.kerman.core.maps.objects.RectangleMapObject;
import com.kerman.core.maps.objects.TextMapObject;
import com.kerman.core.maps.tiled.TiledMapTileLayer.Cell;
import com.kerman.core.maps.tiled.objects.TiledMapTileMapObject;
import com.kerman.core.maps.tiled.tiles.AnimatedTiledMapTile;
import com.kerman.core.maps.tiled.tiles.StaticTiledMapTile;
import com.kerman.core.math.Polygon;
import com.kerman.core.math.Polyline;
import com.kerman.core.utils.KermanArray;
import com.kerman.core.utils.Base64Coder;
...
import com.kerman.core.utils.SerializationException;
import com.kerman.core.utils.StreamUtils;
import com.kerman.core.utils.XmlReader;
...
/**
 * Info : This class was inspired by "com.badlogic.gdx.maps.tiled.BaseTmxMapLoader".
 */
public abstract class BaseTmxMapLoader<P extends BaseTiledMapLoader.Parameters> extends BaseTiledMapLoader<P> {
    ...
    static public int[] getTileIds(Element element, int width, int height) {
        ...
        if (encoding == null) { // no 'encoding' attribute means that the encoding is XML
            throw new KermanRuntimeException("Unsupported encoding (XML) for TMX Layer Data");
        }
            ...
            if (true) if (encoding.equals("base64")) {
                ...
                try {
                    ...
                    if (compression == null)
                        ...
                        throw new KermanRuntimeException("Unrecognised compression (" + compression + ") for TMX Layer Data");
                    ...
                    for (int y = 0; y < height; y++) {
                        for (int x = 0; x < width; x++) {
                            ...
                            if (read != temp.length)
                                throw new KermanRuntimeException("Error Reading TMX Layer Data: Premature end of tile data");
                            ids[y * width + x] = unsignedByteToInt(temp[0]) | unsignedByteToInt(temp[1]) << 8
                                    | unsignedByteToInt(temp[2]) << 16 | unsignedByteToInt(temp[3]) << 24;
                        }
                    }
                } catch (IOException e) {
                    throw new KermanRuntimeException("Error Reading TMX Layer Data - IOException: " + e.getMessage());
                } finally {
                ...
                // or another editor
                throw new KermanRuntimeException("Unrecognised encoding (" + encoding + ") for TMX Layer Data");
            }
        }
        return ids;
    }
    ...
    @Override
    public KermanArray<AssetDescriptor> getDependencies(String fileName, FileHandle tmxFile, P parameter) {
    ...
    protected TiledMap loadTiledMap(FileHandle tmxFile, P parameter, ImageResolver imageResolver) {
        ...
        this.runOnEndOfLoadTiled = new KermanArray<>();
        ...
        KermanArray<Element> tilesets = root.getChildrenByName("tileset");
        ...
        final KermanArray<MapGroupLayer> groups = map.getLayers().getByType(MapGroupLayer.class);
    ...
    protected Element resolveTemplateObject(TiledMap map, MapLayer layer, Element mapElement, FileHandle tmxFile) {
        ...
        if (templateElement == null) {
            ...
            try {
                ...
                throw new KermanRuntimeException("Error parsing template file: " + txFileName, e);
            }
    ...
    protected void loadClassProperties(String className, MapProperties classProperties, XmlReader.Element classElement) {
        ...
        KermanArray<ProjectClassMember> projectClassMembers = projectClassInfo.get(className);
        if (projectClassMembers == null) {
            throw new KermanRuntimeException("There is no class with name '" + className + "' in given Tiled project file.");
        }
        ...
        if (element.getName().equals("tileset")) {
            ...
            if (source != null) {
                ...
                try {
                    ...
                    throw new KermanRuntimeException("Error parsing external tileset.");
                }
            } else {
                Element imageElement = element.getChildByName("image");
                if (imageElement != null) {
            ...
            // Tiles
            KermanArray<Element> tileElements = element.getChildrenByName("tile");
            ...
            KermanArray<AnimatedTiledMapTile> animatedTiles = new KermanArray<>();
    ...
    protected abstract void addStaticTiles(FileHandle tmxFile, ImageResolver imageResolver, TiledMapTileSet tileset,
                                           Element element, KermanArray<Element> tileElements, String name, int firstgid, int tilewidth, int tileheight, int spacing,
    ...
    protected AnimatedTiledMapTile createAnimatedTile(TiledMapTileSet tileSet, TiledMapTile tile, Element tileElement,
        ...
        if (animationElement != null) {
            KermanArray<StaticTiledMapTile> staticTiles = new KermanArray<StaticTiledMapTile>();
            KermanIntArray intervals = new KermanIntArray();
