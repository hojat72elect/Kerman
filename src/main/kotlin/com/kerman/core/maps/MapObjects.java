...
/**
 * Info : This class was inspired by "com.badlogic.gdx.maps.MapObjects".
 * <p>
 * Collection of MapObject instances.
 */
public class MapObjects implements Iterable<MapObject> {

    private final KermanArray<MapObject> objects;

    /**
     * Creates an empty set of MapObject instances
     */
    public MapObjects() {
        objects = new KermanArray<>();
    }
     ...
     */
    public <T extends MapObject> KermanArray<T> getByType(Class<T> type) {
     ...
     * @return array filled with all the objects in the collection matching type
     */
    public <T extends MapObject> KermanArray<T> getByType(Class<T> type, KermanArray<T> fill) {
        fill.clear();
        for (int i = 0, n = objects.size; i < n; i++) {
            MapObject object = objects.get(i);
