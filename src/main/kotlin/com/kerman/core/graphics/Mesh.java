...
import com.kerman.core.graphics.VertexAttributes.Usage;
import com.kerman.core.graphics.glutils.IndexArray;
import com.kerman.core.graphics.glutils.IndexBufferObject;
import com.kerman.core.graphics.glutils.IndexBufferObjectSubData;
import com.kerman.core.graphics.glutils.IndexData;
import com.kerman.core.graphics.glutils.InstanceBufferObject;
import com.kerman.core.graphics.glutils.InstanceData;
import com.kerman.core.graphics.glutils.ShaderProgram;
import com.kerman.core.graphics.glutils.VertexArray;
import com.kerman.core.graphics.glutils.VertexBufferObject;
import com.kerman.core.graphics.glutils.VertexBufferObjectSubData;
import com.kerman.core.graphics.glutils.VertexBufferObjectWithVAO;
import com.kerman.core.graphics.glutils.VertexData;
import com.kerman.core.math.Matrix3;
...
/**
 * Info : This class was inspired by "com.badlogic.gdx.graphics.Mesh".
 * <p>
...
public class Mesh implements Disposable {
     ...
     * list of all meshes
     **/
    static final Map<Application, KermanArray<Mesh>> meshes = new HashMap<>();
    ...
    protected Mesh(VertexData vertices, IndexData indices, boolean isVertexArray) {
        ...
        addManagedMesh(Kerman.app, this);
    }
    ...
    private static void addManagedMesh(Application app, Mesh mesh) {
        KermanArray<Mesh> managedResources = meshes.get(app);
        if (managedResources == null) managedResources = new KermanArray<>();
    ...
    public static void invalidateAllMeshes(Application app) {
        KermanArray<Mesh> meshesArray = meshes.get(app);
    ...
    public Mesh enableInstancedRendering(boolean isStatic, int maxInstances, VertexAttribute... attributes) {
        if (!isInstanced) {
            ...
            throw new KermanRuntimeException("Trying to enable InstancedRendering on same Mesh instance twice."
    ...
    public Mesh setInstanceData(float[] instanceData, int offset, int count) {
        if (instances != null) {
            ...
            throw new KermanRuntimeException("An InstanceBufferObject must be set before setting instance data!");
        }
        return this;
    }
    ...
    public void render(ShaderProgram shader, int primitiveType, int offset, int count, boolean autoBind) {
        ...
        if (isVertexArray) {
            if (indices.getNumIndices() > 0) {
                ...
                Kerman.gl20.glDrawElements(primitiveType, count, GLES20.GL_UNSIGNED_SHORT, buffer);
                ((Buffer) buffer).position(oldPosition);
            } else {
                Kerman.gl20.glDrawArrays(primitiveType, offset, count);
            }
        } else {
            int numInstances = 0;
            ...
            if (indices.getNumIndices() > 0) {
                if (count + offset > indices.getNumMaxIndices()) {
                    throw new KermanRuntimeException("Mesh attempting to access memory outside of the index buffer (count: " + count
                            + ", offset: " + offset + ", max: " + indices.getNumMaxIndices() + ")");
                }

                if (isInstanced && numInstances > 0) {
                    Kerman.gl30.glDrawElementsInstanced(primitiveType, count, GLES20.GL_UNSIGNED_SHORT, offset * 2, numInstances);
                } else {
                    Kerman.gl20.glDrawElements(primitiveType, count, GLES20.GL_UNSIGNED_SHORT, offset * 2);
                }
            } else {
                if (isInstanced && numInstances > 0) {
                    Kerman.gl30.glDrawArraysInstanced(primitiveType, offset, count, numInstances);
                } else {
                    Kerman.gl20.glDrawArrays(primitiveType, offset, count);
                }
            }
     ...
     */
    public void dispose() {
        if (meshes.get(Kerman.app) != null) meshes.get(Kerman.app).removeValue(this, true);
        vertices.dispose();
     ...
     * {@link KermanRuntimeException} is thrown. This method creates a new BoundingBox instance.
     *
    ...
    /**
     * Calculates the {@link BoundingBox} of the vertices contained in this mesh. In case no vertices are defined yet a
     * {@link KermanRuntimeException} is thrown.
     *
     * @param bbox the bounding box to store the result in.
     */
    public void calculateBoundingBox(BoundingBox bbox) {
        final int numVertices = getNumVertices();
        if (numVertices == 0) throw new KermanRuntimeException("No vertices defined");
    ...
    public BoundingBox extendBoundingBox(final BoundingBox out, int offset, int count, final Matrix4 transform) {
        ...
        if (offset < 0 || count < 1 || offset + count > max)
            throw new KermanRuntimeException("Invalid part specified ( offset=" + offset + ", count=" + count + ", max=" + max + " )");
    ...
    public float calculateRadiusSquared(final float centerX, final float centerY, final float centerZ, int offset, int count,
        ...
        if (offset < 0 || count < 1 || offset + count > numIndices) throw new KermanRuntimeException("Not enough indices");
