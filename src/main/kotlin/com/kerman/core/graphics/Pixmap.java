...
import com.kerman.core.graphics.k2d.Kerman2DPixmap;
...
/**
 * Info : This class was inspired by "com.badlogic.gdx.graphics.Pixmap".
 *
 * <p>
 ...
 * <p>
 * By default all functions of this class use blending. You can disable blending with {@link Pixmap#setBlending(Blending)}, which may reduce
...
public class Pixmap implements Disposable {
    ...
    public Pixmap(int width, int height, Format format) {
        pixmap = new Kerman2DPixmap(width, height, Format.toKerman2DPixmapFormat(format));
        setColor(0, 0, 0, 0);
        fill();
    }
    ...
    public Pixmap(byte[] encodedData, int offset, int len) {
        try {
            pixmap = new Kerman2DPixmap(encodedData, offset, len, 0);
        } catch (IOException e) {
            throw new KermanRuntimeException("Couldn't load pixmap from image data", e);
        }
    }
    ...
    public Pixmap(ByteBuffer encodedData, int offset, int len) {
        if (!encodedData.isDirect()) throw new KermanRuntimeException("Couldn't load pixmap from non-direct ByteBuffer");
        try {
            pixmap = new Kerman2DPixmap(encodedData, offset, len, 0);
        } catch (IOException e) {
            throw new KermanRuntimeException("Couldn't load pixmap from image data", e);
    ...
    public Pixmap(FileHandle file) {
        try {
            ...
            pixmap = new Kerman2DPixmap(bytes, 0, bytes.length, 0);
        } catch (Exception e) {
            throw new KermanRuntimeException("Couldn't load file: " + file, e);
        }
    }

    /**
     * Constructs a new Pixmap from a {@link Kerman2DPixmap}.
     */
    public Pixmap(Kerman2DPixmap pixmap) {
        this.pixmap = pixmap;
    }
    ...
    public static Pixmap createFromFrameBuffer(int x, int y, int w, int h) {
        Kerman.gl.glPixelStorei(GLES20.GL_PACK_ALIGNMENT, 1);

        final Pixmap pixmap = new Pixmap(w, h, Format.RGBA8888);
        ByteBuffer pixels = pixmap.getPixels();
        Kerman.gl.glReadPixels(x, y, w, h, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE, pixels);
    ...
    public static void downloadFromUrl(String url, final DownloadPixmapResponseListener responseListener) {
        ...
        request.setUrl(url);
        Kerman.net.sendHttpRequest(request, new Net.HttpResponseListener() {
            @Override
            public void handleHttpResponse(@NotNull Net.HttpResponse httpResponse) {
                final byte[] result = httpResponse.getResult();
                Kerman.app.postRunnable(() -> {
                    try {
    ...
    public void dispose() {
        if (disposed) {
            Kerman.app.error("Pixmap", "Pixmap already disposed!");
            return;
        }
        pixmap.dispose();
        disposed = true;
    ...
    public ByteBuffer getPixels() {
        if (disposed) throw new KermanRuntimeException("Pixmap already disposed");
    ...
    public void setPixels(ByteBuffer pixels) {
        if (!pixels.isDirect()) throw new KermanRuntimeException("Couldn't setPixels from non-direct ByteBuffer");
    ...
    public Format getFormat() {
        return Format.fromKerman2DPixmapFormat(pixmap.getFormat());
    }
    ...
    public void setFilter(Filter filter) {
        this.filter = filter;
        pixmap.setScale(filter == Filter.NearestNeighbour ? Kerman2DPixmap.GDX2D_SCALE_NEAREST : Kerman2DPixmap.GDX2D_SCALE_LINEAR);
    }
    ...
    public enum Format {
        ...
        public static int toKerman2DPixmapFormat(Format format) {
            if (format == Alpha) return Kerman2DPixmap.GDX2D_FORMAT_ALPHA;
            if (format == Intensity) return Kerman2DPixmap.GDX2D_FORMAT_ALPHA;
            if (format == LuminanceAlpha) return Kerman2DPixmap.GDX2D_FORMAT_LUMINANCE_ALPHA;
            if (format == RGB565) return Kerman2DPixmap.GDX2D_FORMAT_RGB565;
            if (format == RGBA4444) return Kerman2DPixmap.GDX2D_FORMAT_RGBA4444;
            if (format == RGB888) return Kerman2DPixmap.GDX2D_FORMAT_RGB888;
            if (format == RGBA8888) return Kerman2DPixmap.GDX2D_FORMAT_RGBA8888;
            throw new KermanRuntimeException("Unknown Format: " + format);
        }

        public static Format fromKerman2DPixmapFormat(int format) {
            if (format == Kerman2DPixmap.GDX2D_FORMAT_ALPHA) return Alpha;
            if (format == Kerman2DPixmap.GDX2D_FORMAT_LUMINANCE_ALPHA) return LuminanceAlpha;
            if (format == Kerman2DPixmap.GDX2D_FORMAT_RGB565) return RGB565;
            if (format == Kerman2DPixmap.GDX2D_FORMAT_RGBA4444) return RGBA4444;
            if (format == Kerman2DPixmap.GDX2D_FORMAT_RGB888) return RGB888;
            if (format == Kerman2DPixmap.GDX2D_FORMAT_RGBA8888) return RGBA8888;
            throw new KermanRuntimeException("Unknown Kerman2DPixmap Format: " + format);
        }

        public static int toGlFormat(Format format) {
            return Kerman2DPixmap.toGlFormat(toKerman2DPixmapFormat(format));
        }

        public static int toGlType(Format format) {
            return Kerman2DPixmap.toGlType(toKerman2DPixmapFormat(format));
        }
    }
