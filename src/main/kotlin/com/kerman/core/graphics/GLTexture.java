...
import com.kerman.core.graphics.TextureData.TextureDataType;
import com.kerman.core.graphics.glutils.MipMapGenerator;
...
/**
 * Info : This abstract class was inspired by "com.badlogic.gdx.graphics.GLTexture".
 * <p>
 * This class represents an OpenGL texture by its target and handle. Keeps track of its state like the TextureFilter and
...
public abstract class GLTexture implements Disposable {
    ...
    public GLTexture(int glTarget) {
        this(glTarget, Kerman.gl.glGenTexture());
    }
    ...
    public static float getMaxAnisotropicFilterLevel() {
        ...
        if (Kerman.graphics.supportsExtension("GL_EXT_texture_filter_anisotropic")) {
            FloatBuffer buffer = BufferUtils.newFloatBuffer(16);
            ((Buffer) buffer).position(0);
            ((Buffer) buffer).limit(buffer.capacity());
            Kerman.gl20.glGetFloatv(GLES20.GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT, buffer);
    ...
    public static void uploadImageData(int target, TextureData data, int miplevel) {
        ...
        Kerman.gl.glPixelStorei(GLES20.GL_UNPACK_ALIGNMENT, 1);
        if (data.useMipMaps()) {
            MipMapGenerator.generateMipMap(target, pixmap, pixmap.getWidth(), pixmap.getHeight());
        } else {
            Kerman.gl.glTexImage2D(target, miplevel, pixmap.getGLInternalFormat(), pixmap.getWidth(), pixmap.getHeight(), 0,
     ...
     */
    public void bind() {
        Kerman.gl.glBindTexture(glTarget, glHandle);
    }
    ...
    public void bind(int unit) {
        Kerman.gl.glActiveTexture(GLES20.GL_TEXTURE0 + unit);
        Kerman.gl.glBindTexture(glTarget, glHandle);
    }
    ...
    public void unsafeSetWrap(TextureWrap u, TextureWrap v, boolean force) {
        if (u != null && (force || uWrap != u)) {
            Kerman.gl.glTexParameteri(glTarget, GLES20.GL_TEXTURE_WRAP_S, u.getGLEnum());
            uWrap = u;
        }
        if (v != null && (force || vWrap != v)) {
            Kerman.gl.glTexParameteri(glTarget, GLES20.GL_TEXTURE_WRAP_T, v.getGLEnum());
            vWrap = v;
        }
    }
    ...
    public void unsafeSetFilter(TextureFilter minFilter, TextureFilter magFilter, boolean force) {
        if (minFilter != null && (force || this.minFilter != minFilter)) {
            Kerman.gl.glTexParameteri(glTarget, GLES20.GL_TEXTURE_MIN_FILTER, minFilter.getGLEnum());
            this.minFilter = minFilter;
        }
        if (magFilter != null && (force || this.magFilter != magFilter)) {
            Kerman.gl.glTexParameteri(glTarget, GLES20.GL_TEXTURE_MAG_FILTER, magFilter.getGLEnum());
    ...
    public float unsafeSetAnisotropicFilter(float level, boolean force) {
        ...
        Kerman.gl20.glTexParameterf(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_MAX_ANISOTROPY_EXT, level);
    ...
    protected void delete() {
        if (glHandle != 0) {
            Kerman.gl.glDeleteTexture(glHandle);
            glHandle = 0;
        }
    }
