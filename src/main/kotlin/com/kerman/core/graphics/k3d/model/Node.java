...
/**
 * Info : This class was inspired by "com.badlogic.gdx.graphics.g3d.model.Node".
 * <p>
...
public class Node {
    ...
    private final KermanArray<Node> children = new KermanArray<Node>(2);
    /**
    ...
    public KermanArray<NodePart> parts = new KermanArray<NodePart>(2);
     ...
     */
    public static Node getNode(final KermanArray<Node> nodes, final String id, boolean recursive, boolean ignoreCase) {
    ...
    public <T extends Node> int insertChild(int index, final T child) {
        for (Node p = this; p != null; p = p.getParent()) {
            if (p == child) throw new KermanRuntimeException("Cannot add a parent as a child");
        }
        Node p = child.getParent();
        if (p != null && !p.removeChild(child)) throw new KermanRuntimeException("Could not remove child from its current parent");
