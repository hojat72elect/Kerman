...
import com.kerman.core.utils.IntIntMap;
import org.jetbrains.annotations.NotNull;

/**
 * Info : This abstract class was inspired by "com.badlogic.gdx.graphics.g3d.shaders.BaseShader".
 * <p>
...
public abstract class BaseShader implements Shader {
    private final KermanArray<String> uniforms = new KermanArray<>();
    private final KermanArray<Validator> validators = new KermanArray<>();
    private final KermanArray<Setter> setters = new KermanArray<>();
    private final KermanIntArray globalUniforms = new KermanIntArray();
    private final KermanIntArray localUniforms = new KermanIntArray();
    private final IntIntMap attributes = new IntIntMap();
    private final IntIntMap instancedAttributes = new IntIntMap();
    private final KermanIntArray tempArray = new KermanIntArray();
    private final KermanIntArray tempArray2 = new KermanIntArray();
    ...
    public int register(final String alias, final Validator validator, final Setter setter) {
        if (locations != null) throw new KermanRuntimeException("Cannot register an uniform after initialization");
    ...
    public void init(final ShaderProgram program, final Renderable renderable) {
        if (locations != null) throw new KermanRuntimeException("Already initialized");
        if (!program.isCompiled()) throw new KermanRuntimeException(program.getLog());
