...
/**
 * Info : This class was inspired by "com.badlogic.gdx.graphics.glutils.FrameBufferCubemap".
 * <p>
...
public class FrameBufferCubemap extends GLFrameBuffer<Cubemap> {
     ...
     * @throws com.kerman.core.utils.KermanRuntimeException in case the FrameBuffer could not be created
     */
    ...
    protected void attachFrameBufferColorTexture(Cubemap texture) {
        ...
        for (Cubemap.CubemapSide side : sides) {
            gl.glFramebufferTexture2D(GLES20.GL_FRAMEBUFFER, GLES20.GL_COLOR_ATTACHMENT0, side.glEnum, glHandle, 0);
        }
    }

    /**
     * Makes the frame buffer current so everything gets drawn to it, must be followed by call to either {@link #nextSide()} or
     * {@link #bindSide(com.kerman.core.graphics.Cubemap.CubemapSide)} to activate the side to render onto.
     */
    @Override
    public void bind() {
        currentSide = -1;
    ...
    public boolean nextSide() {
        if (currentSide > 5) {
            throw new KermanRuntimeException("No remaining sides.");
    ...
    protected void bindSide(final Cubemap.CubemapSide side) {
        Kerman.gl20.glFramebufferTexture2D(GLES20.GL_FRAMEBUFFER, GLES20.GL_COLOR_ATTACHMENT0, side.glEnum,
