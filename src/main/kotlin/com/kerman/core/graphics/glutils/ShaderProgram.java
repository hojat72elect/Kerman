...
import com.kerman.core.math.Vector4;
...
/**
 * Info : This class was inspired by "com.badlogic.gdx.graphics.glutils.ShaderProgram".
 * <p>
 ...
 * {@link ShaderProgram#bind()} function must be used which effectively binds the program.
 * </p>
 *
 * <p>
...
public class ShaderProgram implements Disposable {
     ...
     **/
    private final static ObjectMap<Application, KermanArray<ShaderProgram>> shaders = new ObjectMap<Application, KermanArray<ShaderProgram>>();
    /**
    ...
    public ShaderProgram(String vertexShader, String fragmentShader) {
        ...
        if (isCompiled()) {
            fetchAttributes();
            fetchUniforms();
            addManagedShader(Kerman.app, this);
        }
    }
    ...
    public static void invalidateAllShaderPrograms(Application app) {
        ...
        KermanArray<ShaderProgram> shaderArray = shaders.get(app);
    ...
    public static int getNumManagedShaderPrograms() {
        return shaders.get(Kerman.app).size;
    }
    ...
    private int loadShader(int type, String source) {
        ...
        gl.glGetShaderiv(shader, GLES20.GL_COMPILE_STATUS, intbuf);
    ...
    private int linkProgram(int program) {
        ...
        gl.glGetProgramiv(program, GLES20.GL_LINK_STATUS, intbuf);
        int linked = intbuf.get(0);
        if (linked == 0) {
            log = Kerman.gl20.glGetProgramInfoLog(program);
            return -1;
        }
    ...
    public int fetchUniformLocation(String name, boolean pedantic) {
        ...
        if ((location = uniforms.get(name, -2)) == -2) {
            location = Kerman.gl20.glGetUniformLocation(program, name);
    ...
    public void dispose() {
        ...
        if (shaders.get(Kerman.app) != null) shaders.get(Kerman.app).removeValue(this, true);
    }
    ...
    private void addManagedShader(Application app, ShaderProgram shaderProgram) {
        KermanArray<ShaderProgram> managedResources = shaders.get(app);
        if (managedResources == null) managedResources = new KermanArray<ShaderProgram>();
    ...
    private void fetchUniforms() {
        ((Buffer) params).clear();
        Kerman.gl20.glGetProgramiv(program, GLES20.GL_ACTIVE_UNIFORMS, params);
        ...
        for (int i = 0; i < numUniforms; i++) {
            ...
            params.put(0, 1);
            ((Buffer) type).clear();
            String name = Kerman.gl20.glGetActiveUniform(program, i, params, type);
            int location = Kerman.gl20.glGetUniformLocation(program, name);
            uniforms.put(name, location);
    ...
    private void fetchAttributes() {
        ((Buffer) params).clear();
        Kerman.gl20.glGetProgramiv(program, GLES20.GL_ACTIVE_ATTRIBUTES, params);
        ...
        for (int i = 0; i < numAttributes; i++) {
            ...
            params.put(0, 1);
            ((Buffer) type).clear();
            String name = Kerman.gl20.glGetActiveAttrib(program, i, params, type);
            int location = Kerman.gl20.glGetAttribLocation(program, name);
            attributes.put(name, location);
