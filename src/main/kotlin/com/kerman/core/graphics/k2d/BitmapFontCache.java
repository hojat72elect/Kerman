...
import com.kerman.core.utils.FlushablePool;
...
/**
 * Info : This class was inspired by "com.badlogic.gdx.graphics.g2d.BitmapFontCache".
 * <p>
 * Caches glyph geometry for a BitmapFont, providing a fast way to render static text. This removes the need to compute the glyph
 * geometry each frame.
 */
public class BitmapFontCache {
    ...
    private final KermanArray<GlyphLayout> layouts = new KermanArray<>(1);
     ...
     */
    private KermanIntArray[] pageGlyphIndices;
    /**
    ...
    public BitmapFontCache(BitmapFont font, boolean integer) {
        ...
        if (pageCount > 1) {
            ...
            pageGlyphIndices = new KermanIntArray[pageCount];
            for (int i = 0, n = pageGlyphIndices.length; i < n; i++)
                pageGlyphIndices[i] = new KermanIntArray();
        }
    ...
    public void tint(Color tint) {
        ...
        for (int i = 0, n = layouts.size; i < n; i++) {
            ...
            KermanIntArray colors = layout.colors;
    ...
    public void setColors(float color, int start, int end) {
        ...
        for (int i = 0; i < pageCount; i++) {
            float[] vertices = pageVertices[i];
            KermanIntArray glyphIndices = pageGlyphIndices[i];
    ...
    public void draw(Batch spriteBatch) {
        KermanArray<TextureRegion> regions = font.getRegions();
    ...
    private void requireGlyphs(GlyphLayout layout) {
        if (pageVertices.length == 1) {
            ...
            for (int i = 0, n = layout.runs.size; i < n; i++) {
                KermanArray<Glyph> glyphs = layout.runs.get(i).glyphs;
    ...
    private void setPageCount(int pageCount) {
        ...
        KermanIntArray[] newPageGlyphIndices = new KermanIntArray[pageCount];
        ...
        for (int i = pageGlyphIndicesLength; i < pageCount; i++)
            newPageGlyphIndices[i] = new KermanIntArray();
    ...
    public KermanArray<GlyphLayout> getLayouts() {
        return layouts;
    }
}
