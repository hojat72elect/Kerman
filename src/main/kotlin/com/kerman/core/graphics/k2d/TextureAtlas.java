...
/**
 * Info : This class was inspired by the "com.badlogic.gdx.graphics.g2d.TextureAtlas".
 * <p>
...
public class TextureAtlas implements Disposable {
    ...
    private final KermanArray<AtlasRegion> regions = new KermanArray<>();
    ...
    public TextureAtlas(String internalPackFile) {
        this(Kerman.files.internal(internalPackFile));
    }
     ...
     */
    public KermanArray<AtlasRegion> getRegions() {
        return regions;
    }
     ...
     * times.
     */
    public KermanArray<AtlasRegion> findRegions(String name) {
        KermanArray<AtlasRegion> matched = new KermanArray<>(AtlasRegion[]::new);
        for (int i = 0, n = regions.size; i < n; i++) {
            AtlasRegion region = regions.get(i);
     ...
     */
    public KermanArray<Sprite> createSprites() {
        KermanArray<Sprite> sprites = new KermanArray<>(true, regions.size, Sprite[]::new);
        for (int i = 0, n = regions.size; i < n; i++)
     ...
     * @see #createSprite(String)
     */
    public KermanArray<Sprite> createSprites(String name) {
        KermanArray<Sprite> matched = new KermanArray<>(Sprite[]::new);
        for (int i = 0, n = regions.size; i < n; i++) {
            AtlasRegion region = regions.get(i);
    ...
    static public class TextureAtlasData {
        final KermanArray<Page> pages = new KermanArray<>();
        final KermanArray<Region> regions = new KermanArray<>();
        ...
        public void load(FileHandle packFile, FileHandle imagesDir, boolean flip) {
            ...
            try {
                ...
                Page page = null;
                KermanArray<String> names = null;
                KermanArray<int[]> values = null;
                while (true) {
                    if (line == null) break;
                    if (line.trim().length() == 0) {
                        ...
                        while (true) {
                            ...
                            if (field != null)
                                ...
                                if (names == null) {
                                    names = new KermanArray(8);
                                    values = new KermanArray(8);
                                }
                                names.add(entry[0]);
        ...
        public KermanArray<Page> getPages() {
            return pages;
        }

        public KermanArray<Region> getRegions() {
            return regions;
        }
