...
/**
 * Info : This class was inspired by "com.badlogic.gdx.graphics.g2d.Sprite".
 * <p>
...
public class Sprite extends TextureRegion {
    ...
    public Rectangle getBoundingRectangle() {
        ...
        minx = Math.min(minx, vertices[Batch.X2]);
        minx = Math.min(minx, vertices[Batch.X3]);
        minx = Math.min(minx, vertices[Batch.X4]);

        maxx = Math.max(maxx, vertices[Batch.X2]);
        maxx = Math.max(maxx, vertices[Batch.X3]);
        maxx = Math.max(maxx, vertices[Batch.X4]);

        miny = Math.min(miny, vertices[Batch.Y2]);
        miny = Math.min(miny, vertices[Batch.Y3]);
        miny = Math.min(miny, vertices[Batch.Y4]);

        maxy = Math.max(maxy, vertices[Batch.Y2]);
        maxy = Math.max(maxy, vertices[Batch.Y3]);
        maxy = Math.max(maxy, vertices[Batch.Y4]);
