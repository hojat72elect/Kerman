...
import com.kerman.core.graphics.k2d.GlyphLayout.GlyphRun;
...
import com.kerman.core.utils.KermanFloatArray;
...
/**
 * Info : This class was inspired by "com.badlogic.gdx.graphics.g2d.BitmapFont".
 * <p>
...
public class BitmapFont implements Disposable {
    ...
    KermanArray<TextureRegion> regions;
    boolean integer;
     ...
     */
    public BitmapFont() {
        this(Kerman.files.classpath("com/badlogic/gdx/utils/lsans-15.fnt"), Kerman.files.classpath("com/badlogic/gdx/utils/lsans-15.png"),
                false, true);
    }
    ...
    public BitmapFont(BitmapFontData data, TextureRegion region, boolean integer) {
        this(data, region != null ? KermanArray.with(region) : null, integer);
    }
     ...
     * @param integer If true, rendering positions will be at integer values to avoid filtering artifacts.
     */
    public BitmapFont(BitmapFontData data, KermanArray<TextureRegion> pageRegions, boolean integer) {
        ...
        if (pageRegions == null || pageRegions.size == 0) {
            ...
            regions = new KermanArray(n);
            for (int i = 0; i < n; i++) {
                FileHandle file;
                if (data.fontFile == null)
                    file = Kerman.files.internal(data.imagePaths[i]);
                else
                    file = Kerman.files.getFileHandle(data.imagePaths[i], data.fontFile.type());
     ...
     */
    public KermanArray<TextureRegion> getRegions() {
        return regions;
    }
    ...
    static public class BitmapFontData {
        ...
        public void load(FileHandle fontFile, boolean flip) {
            ...
            try {
                ...
                if (line == null) throw new KermanRuntimeException("File is empty.");

                line = line.substring(line.indexOf("padding=") + 8);
                String[] padding = line.substring(0, line.indexOf(' ')).split(",", 4);
                if (padding.length != 4) throw new KermanRuntimeException("Invalid padding.");
                ...
                if (line == null) throw new KermanRuntimeException("Missing common header.");
                String[] common = line.split(" ", 9); // At most we want the 6th element; i.e. "page=N"

                // At least lineHeight and base are required.
                if (common.length < 3) throw new KermanRuntimeException("Invalid common header.");

                if (!common[1].startsWith("lineHeight=")) throw new KermanRuntimeException("Missing: lineHeight");
                lineHeight = Integer.parseInt(common[1].substring(11));

                if (!common[2].startsWith("base=")) throw new KermanRuntimeException("Missing: base");
                ...
                for (int p = 0; p < pageCount; p++) {
                    ...
                    line = reader.readLine();
                    if (line == null) throw new KermanRuntimeException("Missing additional page definitions.");
                    ...
                    if (matcher.find()) {
                        ...
                        try {
                            ...
                            if (pageID != p) throw new KermanRuntimeException("Page IDs must be indices starting at 0: " + id);
                        } catch (NumberFormatException ex) {
                            throw new KermanRuntimeException("Invalid page id: " + id, ex);
                        }
                    }

                    matcher = Pattern.compile(".*file=\"?([^\"]+)\"?").matcher(line);
                    if (!matcher.find()) throw new KermanRuntimeException("Missing: file");
                ...
                throw new KermanRuntimeException("Error loading font file: " + fontFile, ex);
            } finally {
        ...
        public Glyph getFirstGlyph() {
            ...
            throw new KermanRuntimeException("No glyphs found.");
        }
        ...
        public void getGlyphs(GlyphRun run, CharSequence str, int start, int end, Glyph lastGlyph) {
            ...
            KermanArray<Glyph> glyphs = run.glyphs;
            KermanFloatArray xAdvances = run.xAdvances;
         ...
         */
        public int getWrapIndex(KermanArray<Glyph> glyphs, int start) {
            int i = start - 1;
