...
import com.kerman.core.graphics.glutils.FileTextureData;
import com.kerman.core.graphics.glutils.PixmapTextureData;
...
/**
 * Info : This class was inspired by "com.badlogic.gdx.graphics.Texture".
 * <p>
 ...
 * to another application or receives an incoming phone call. Managed textures get reloaded automatically.
 * <p>
...
public class Texture extends GLTexture {
    final static Map<Application, KermanArray<Texture>> managedTextures = new HashMap<>();
    ...
    public Texture(String internalPath) {
        this(Kerman.files.internal(internalPath));
    }
    ...
    public Texture(TextureData data) {
        this(GLES20.GL_TEXTURE_2D, Kerman.gl.glGenTexture(), data);
    }

    protected Texture(int glTarget, int glHandle, TextureData data) {
        super(glTarget, glHandle);
        load(data);
        if (data.isManaged()) addManagedTexture(Kerman.app, this);
    }

    private static void addManagedTexture(Application app, Texture texture) {
        KermanArray<Texture> managedTextureArray = managedTextures.get(app);
        if (managedTextureArray == null) managedTextureArray = new KermanArray<>();
    ...
    public static void invalidateAllTextures(Application app) {
        ...
        if (assetManager == null) {
            ...
            // asset manager.
            KermanArray<Texture> textures = new KermanArray<>(managedTextureArray);
            for (Texture texture : textures) {
                ...
                if (fileName == null) {
                    ...
                    texture.glHandle = Kerman.gl.glGenTexture();
    ...
    public static int getNumManagedTextures() {
        return managedTextures.get(Kerman.app).size;
    }

    public void load(TextureData data) {
        if (this.data != null && data.isManaged() != this.data.isManaged())
            throw new KermanRuntimeException("New data must have the same managed status as the old data");
        this.data = data;
        ...
        Kerman.gl.glBindTexture(glTarget, 0);
    }
    ...
    protected void reload() {
        if (!isManaged()) throw new KermanRuntimeException("Tried to reload unmanaged Texture");
        glHandle = Kerman.gl.glGenTexture();
        load(data);
    }
    ...
    public void draw(Pixmap pixmap, int x, int y) {
        if (data.isManaged()) throw new KermanRuntimeException("can't draw to a managed texture");

        bind();
        Kerman.gl.glTexSubImage2D(glTarget, 0, x, y, pixmap.getWidth(), pixmap.getHeight(), pixmap.getGLFormat(), pixmap.getGLType(),
                pixmap.getPixels());
    }
    ...
    public void dispose() {
        ...
        delete();
        if (data.isManaged()) if (managedTextures.get(Kerman.app) != null) managedTextures.get(Kerman.app).removeValue(this, true);
    }
