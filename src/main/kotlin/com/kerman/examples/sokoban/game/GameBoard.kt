...
import com.kerman.examples.sokoban.objects.Box
import com.kerman.examples.sokoban.objects.Player
import com.kerman.examples.sokoban.objects.TargetPlatform
import com.kerman.examples.sokoban.objects.Tile
import com.kerman.examples.sokoban.objects.Wall
...
class GameBoard : Group() {
    ...
    private var playerMoves: KermanArray<Vector2> = KermanArray()

    // X previous position, Y current position.
    private var boxMoves: KermanArray<Vector2?> = KermanArray()

    private var tiles: KermanArray<Tile> = KermanArray(25 * 15)
    ...
    override fun act(delta: Float) {
        ...
        if (state == STATE_RUNNING) {
            ...
            if (state == STATE_RUNNING) time += Kerman.graphics.getRawDeltaTime()
        }
    }
    ...
    private fun isPositionEmpty(position: Int): Boolean {
        val iterator = KermanArray.ArrayIterator(tiles)
    ...
    private fun getBoxInPosition(position: Int): Box? {
        val ite = KermanArray.ArrayIterator(tiles)
        while (ite.hasNext()) {
            val obj = ite.next()
